...
- 关键词通道：PostgreSQL 自带 FTS——`tsvector` 列 + 触发器自动维护 + GIN 索引 + `ts_rank` 打分
- **架构决策（待讨论）**：扩展 Spring AI 默认 `vector_store` 表（加 tsvector 列与触发器），还是自建知识表？倾向扩展 Spring AI 表——能继续用 VectorStore 抽象做相似度检索，FTS 走原生 SQL，服务层融合。零新中间件是面试加分点
